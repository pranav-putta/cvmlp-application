...
class PTGRUDecoder(nn.Module):
    def __init__(self, hidden_size=128, embed_size=4, num_layers=4):
        ...
        self.num_layers = num_layers
        self.gru = nn.GRU(embed_size, hidden_size, num_layers=num_layers, batch_first=True, bidirectional=False)
        self.linear = nn.Sequential(nn.Linear(hidden_size, embed_size))
...
class PTGRUSeq2Seq(nn.Module):
    def __init__(self, hidden_size=128, embed_size=4, num_layers=4, device=None):
        super().__init__()
    ...
    def forward(self, inp_tensor, tgt_tensor=None, teacher_forcing=False, max_len=100):
        ...
        if tgt_tensor is not None:
            ...
            if teacher_forcing:
                ...
                for di in range(target_length - 1):
                    ...
                    loss += F.mse_loss(decoder_output[:, 0, :], tgt_tensor[:, di + 1, :])
            ...
            for di in range(max_len):
                ...
                decoder_input = decoder_output.detach().clone()  # detach from history as input
                decoder_outputs.append(decoder_output)
                if decoder_output == torch.zeros(decoder_output.shape):
                    break
